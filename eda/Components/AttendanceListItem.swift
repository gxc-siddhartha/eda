    ...
    var body: some View {
        HStack {
            ...
            VStack(alignment: .leading, spacing: 0) {
                HStack {
                    Text(
                        formatDateToWeekday(attendance.attendanceDate ?? Date())
                    ).lineLimit(1)
                        .font(.headline)

                    Text(attendance.attendanceType ?? "N/A").font(
                        .system(.subheadline, design: .rounded)
                    ).fontWeight(Font.Weight.semibold).opacity(0.4)
                }.padding(.top, 6)
            ...
            HStack {
                ...
                VStack(alignment: .leading, spacing: -2) {
                    Text(
                        "\(formatDateToTimeAndHour(attendance.attendanceStartTime ?? Date()))"
                    ).font(.caption)
                        .fontWeight(Font.Weight.medium).opacity(0.6).lineLimit(
                            1
                        )
                    Text(
                        "\(formatDateToTimeAndHour(attendance.attendanceEndTime ?? Date()))"
                    )
