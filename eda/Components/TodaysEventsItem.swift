...
struct TodaysEventsItem: View {
    ...
    private var firstScheduleStartTime: String {
        return formatDateToHourAndMinute(todaysSchedules[0]?.scheduleStartTime ?? Date())
    }
    
    
    private var lastScheduleEndTime: String {
        return formatDateToHourAndMinute(todaysSchedules.last??.scheduleEndTime ?? Date())
    }
            ...
            VStack(alignment: .leading, ) {
                ...
                HStack {
                    Image(systemName: "figure.walk")
                        .font(.title3)
                    Text(firstScheduleStartTime).font(.system(.title2, design: .rounded)).fontWeight(Font.Weight.medium)
                    Text("to")
                    Text(lastScheduleEndTime).font(.system(.title2, design: .rounded)).fontWeight(Font.Weight.medium)
                }
            }
            Spacer()
        }.padding(12)
