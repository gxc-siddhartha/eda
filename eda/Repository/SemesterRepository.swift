...
extension UserDefaults {
    ...
    var selectedSemesterID: UUID? {
        get {
            guard let uuidString = string(forKey: Keys.selectedSemesterID)
            else { return nil }
...
class SemesterRepository {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    ...
                    newSemester.passingPercentage =
                        semesterData.passingPercentage
                    ...
                    let mainContextSemester =
                        try self.persistentContainer.viewContext.existingObject(
                            with: newSemester.objectID
                        ) as! Semester

                    self.logger.info(
                        "✅ Semester created successfully: \(semesterData.name)"
                    )
                    continuation.resume(returning: mainContextSemester)

                } catch {
                    self.logger.error(
                        "❌ Failed to create semester: \(error.localizedDescription)"
                    )
                }
            }
        }
    ...
    func updateSemester(semesterId: UUID, semesterData: SemesterData)
        ...
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    ...
                    let fetchRequest: NSFetchRequest<Semester> =
                    ...
                    let semesters = try self.backgroundContext.fetch(
                        fetchRequest
                    )
                    guard let semester = semesters.first else {
                        self.logger.error(
                            "Error updating semester: Semester not found with the id: \(semesterId.uuidString) and name: \(semesterData.name)"
                        )
                        return
                    }
                    ...
                    let mainContextSemester =
                        try self.persistentContainer.viewContext.existingObject(
                            with: semester.objectID
                        ) as! Semester

                    self.logger.info(
                        "✅ Semester updated successfully: \(semesterData.name)"
                    )
                    continuation.resume(returning: mainContextSemester)
        ...
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    ...
                    guard let semester = semesters.first else {
                        self.logger.warning(
                            "⚠️ Semester not found: \(semesterId.uuidString)"
                        )
                        return
                    }
                    ...
                    self.logger.info(
                        "✅ Semester deleted successfully: \(semesterId.uuidString)"
                    )
                    continuation.resume()

                } catch {
                    self.logger.error(
                        "❌ Failed to delete semester: \(error.localizedDescription)"
                    )
    ...
    func updateSelectedSemester(_ semester: Semester) async throws {
        self.logger.debug(
            "🎯 Updating selected semester in UserDefaults: \(semester.semesterName ?? "unknown")"
        )

        guard let semesterId = semester.semesterId else {
            self.logger.error(
                "❌ Failed to update selected semester: Missing semester ID"
            )
            return
        }
        ...
        self.logger.info(
            "✅ Selected semester updated in UserDefaults: \(semester.semesterName ?? "unknown")"
        )
    }
        ...
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    ...
                    let mainContextSemesters = try backgroundSemesters.map {
                        semester in
                        try self.persistentContainer.viewContext.existingObject(
                            with: semester.objectID
                        ) as! Semester
                    ...
                    self.logger.info(
                        "✅ Fetched \(mainContextSemesters.count) semesters"
                    )
                    continuation.resume(returning: mainContextSemesters)

                } catch {
                    self.logger.error(
                        "❌ Failed to fetch semesters: \(error.localizedDescription)"
                    )
                }
            }
        }
                ...
                do {
                    ...
                    if let backgroundSemester = backgroundSemesters.first {
                        ...
                        let mainContextSemester =
                            ...
                            .existingObject(with: backgroundSemester.objectID)
                            as! Semester

                        self.logger.info(
                            "✅ Found semester: \(mainContextSemester.semesterName ?? "unnamed")"
                        )
                        continuation.resume(returning: mainContextSemester)
                    } else {
                        self.logger.info(
                            "ℹ️ No semester found with ID: \(id.uuidString)"
                        )
                    ...
                    self.logger.error(
                        "❌ Failed to fetch semester by ID: \(error.localizedDescription)"
                    )
                }
            }
        }
    ...
    func getSelectedSemester() async throws -> Semester? {
        ...
        guard
            let selectedSemesterID = await MainActor.run(body: {
        ...
        self.logger.debug(
            "📚 Found selected semester ID: \(selectedSemesterID.uuidString)"
        )
        ...
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    ...
                    let existingSemesters = try self.backgroundContext.fetch(
                        fetchRequest
                    )
