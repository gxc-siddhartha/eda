...
class AttendanceRepository {
    ...
    func createAttendance(attendanceData: AttendanceData, for subject: Subject) async throws -> Attendance {
        self.logger.debug("📋 Creating attendance for subject: \(subject.subjectName ?? "unknown")")
        ...
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    ...
                    // Validate time order
                    guard attendanceData.attendanceStartTime < attendanceData.attendanceEndTime else {
                        let error = AttendanceRepositoryError.invalidAttendanceData("Start time must be before end time")
                        self.logger.error("❌ \(error.localizedDescription)")
                        continuation.resume(throwing: error)
                        return
                    }
                    ...
                    if !existingAttendance.isEmpty {
                        ...
                        self.logger.error("   Subject: \(backgroundSubject.subjectName ?? "unknown")")
                        self.logger.error("   Date: \(attendanceData.attendanceDate)")
                        self.logger.error("   Start Time: \(attendanceData.attendanceStartTime)")
                        self.logger.error("   Existing Start Time: \(existingAttendance.first?.attendanceStartTime ?? Date())")
                        
                        let formatter = DateFormatter()
                        formatter.dateStyle = .medium
                        formatter.timeStyle = .short
                        
                        let error = AttendanceRepositoryError.duplicateAttendance("Attendance already exists for \(backgroundSubject.subjectName ?? "this subject") on \(formatter.string(from: attendanceData.attendanceDate)) at \(formatter.string(from: attendanceData.attendanceStartTime))")
                        self.logger.error("❌ \(error.localizedDescription)")
                        continuation.resume(throwing: error)
                        return
                    }
                    ...
                    newAttendance.attendanceStartTime = attendanceData.attendanceStartTime
                    newAttendance.attendanceEndTime = attendanceData.attendanceEndTime
                    newAttendance.attendanceLocation = attendanceData.attendanceLocation?.trimmingCharacters(in: .whitespacesAndNewlines)
                    ...
                    self.logger.info("✅ Attendance created successfully for subject: \(backgroundSubject.subjectName ?? "unknown")")
            ...
            backgroundContext.perform {
                do {
                    ...
                    if let subject = attendance.subject {
                        ...
                        let duplicateCheckRequest: NSFetchRequest<Attendance> = Attendance.fetchRequest()
                        duplicateCheckRequest.predicate = NSPredicate(
                            format: "subject == %@ AND attendanceDate >= %@ AND attendanceDate < %@ AND attendanceStartTime == %@ AND attendanceId != %@",
                            subject,
                            startOfDay as NSDate,
                            endOfDay as NSDate,
                            attendanceData.attendanceStartTime as NSDate,
                        ...
                        let existingAttendance = try self.backgroundContext.fetch(duplicateCheckRequest)
                        if !existingAttendance.isEmpty {
                            let error = AttendanceRepositoryError.duplicateAttendance("Attendance already exists for this subject on this date and time")
                            self.logger.error("❌ \(error.localizedDescription)")
                            continuation.resume(throwing: error)
                            return
                        }
                    ...
                    attendance.attendanceStartTime = attendanceData.attendanceStartTime
                    attendance.attendanceEndTime = attendanceData.attendanceEndTime
                    attendance.attendanceLocation = attendanceData.attendanceLocation?.trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    ...
                    let subjectName = attendance.subject?.subjectName ?? "Unknown Subject"
                    ...
                    self.logger.info("✅ Attendance deleted successfully for subject: \(subjectName)")
                    continuation.resume()
        ...
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    ...
                    let fetchRequest: NSFetchRequest<Attendance> = Attendance.fetchRequest()
                    fetchRequest.predicate = NSPredicate(format: "subject == %@", backgroundSubject)
                    
                    // Sort by attendance date and start time
                    fetchRequest.sortDescriptors = [
                        NSSortDescriptor(key: "attendanceDate", ascending: false), // Most recent first
                        NSSortDescriptor(key: "attendanceStartTime", ascending: true)
                    ]
            ...
            backgroundContext.perform {
                do {
                    ...
                    fetchRequest.predicate = NSPredicate(
                        format: "subject == %@ AND attendanceDate >= %@ AND attendanceDate < %@",
                        backgroundSubject,
                        startOfDay as NSDate,
                        endOfDay as NSDate
                    )
