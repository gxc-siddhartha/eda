...
class SubjectRepository {
    ...
    // MARK: - Create Subject
    func createSubject(subjectData: SubjectData, for semester: Semester)
        async throws -> Subject
    {
        self.logger.debug(
            "📚 Creating subject: \(subjectData.name) for semester: \(semester.semesterName ?? "unknown")"
        )
            ...
            backgroundContext.perform {
                do {
                    ...
                    guard
                        ...
                        let error = SubjectRepositoryError.semesterNotFound(
                    ...
                    let isDuplicate = try self.checkForDuplicateSubjectNameSync(
                        subjectData.name,
                        in: backgroundSemester
                    )
                    if isDuplicate {
                        let error = SubjectRepositoryError.duplicateSubjectName(
                            subjectData.name
                        )
                        self.logger.error("❌ \(error.localizedDescription)")
                        continuation.resume(throwing: error)
                    ...
                    let mainContextSubject =
                        try self.persistentContainer.viewContext.existingObject(
                            with: newSubject.objectID
                        ) as! Subject

                    self.logger.info(
                        "✅ Subject created successfully: \(subjectData.name) in semester: \(backgroundSemester.semesterName ?? "unknown")"
                    )
                    ...
                    let wrappedError = SubjectRepositoryError.coreDataError(
                        "Failed to create subject: \(error.localizedDescription)"
                    )
    ...
    func updateSubject(
        ...
        self.logger.debug(
            "📝 Updating subject: \(subjectId.uuidString) with name: \(subjectData.name)"
        )
        ...
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    ...
                    let subjects = try self.backgroundContext.fetch(
                        fetchRequest
                    )
                    guard let subject = subjects.first else {
                        let error = SubjectRepositoryError.subjectNotFound(
                            "Subject with ID \(subjectId.uuidString) not found"
                        )
                        self.logger.error("❌ \(error.localizedDescription)")
                        continuation.resume(throwing: error)
                        return
                    ...
                    self.logger.info(
                        "✅ Subject updated successfully: \(subjectData.name) in semester: \(backgroundSemester.semesterName ?? "unknown")"
                    )
                    continuation.resume(returning: mainContextSubject)
                    ...
                    let wrappedError = SubjectRepositoryError.coreDataError(
                        "Failed to update subject: \(error.localizedDescription)"
                    )
                    self.logger.error("❌ \(wrappedError.localizedDescription)")
                    continuation.resume(throwing: wrappedError)
                }
        ...
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    ...
                    let semesterName =
                        subject.semester?.semesterName ?? "Unknown Semester"
                    ...
                    self.logger.info(
                        "✅ Subject deleted successfully: \(subjectName) from semester: \(semesterName)"
                    )
                    continuation.resume()
                    ...
                    let wrappedError = SubjectRepositoryError.coreDataError(
                        "Failed to delete subject: \(error.localizedDescription)"
                    )
                    self.logger.error("❌ \(wrappedError.localizedDescription)")
                    continuation.resume(throwing: wrappedError)
                }
    ...
    func getAllSubjects(for semester: Semester) async throws -> [Subject] {
        self.logger.debug(
            "📚 Fetching all subjects for semester: \(semester.semesterName ?? "unknown")"
        )
        ...
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    ...
                    self.logger.info(
                        "✅ Fetched \(mainContextSubjects.count) subjects for semester: \(backgroundSemester.semesterName ?? "unknown")"
                    )
                    ...
                    let wrappedError = SubjectRepositoryError.coreDataError(
                        "Failed to fetch subjects: \(error.localizedDescription)"
                    )
                    self.logger.error("❌ \(wrappedError.localizedDescription)")
                    continuation.resume(throwing: wrappedError)
                }
                ...
                do {
                    ...
                    if let backgroundSubject = backgroundSubjects.first {
                        ...
                        let mainContextSubject =
                            ...
                            .existingObject(with: backgroundSubject.objectID)
                            as! Subject

                        self.logger.info(
                            "✅ Found subject: \(mainContextSubject.subjectName ?? "unnamed")"
                        )
                        continuation.resume(returning: mainContextSubject)
                    } else {
                        self.logger.info(
                            "ℹ️ No subject found with ID: \(id.uuidString)"
                        )
                    ...
                    let wrappedError = SubjectRepositoryError.coreDataError(
                        "Failed to fetch subject by ID: \(error.localizedDescription)"
                    )
                    self.logger.error("❌ \(wrappedError.localizedDescription)")
                    continuation.resume(throwing: wrappedError)
                }
    ...
    private func checkForDuplicateSubjectNameSync(
        ...
        excluding excludedSubject: Subject? = nil
    ) throws -> Bool {
        ...
        if let excludedSubject = excludedSubject {
            predicateComponents.append(
                NSPredicate(
                    format: "subjectId != %@",
                    excludedSubject.subjectId! as CVarArg
                )
            )
        }
    ...
    func checkForDuplicateSubjectName(
        _ name: String,
        in semester: Semester,
        excluding excludedSubjectId: UUID? = nil
    ) async throws -> Bool {
        self.logger.debug(
            "🔍 Checking for duplicate subject name: \(name) in semester: \(semester.semesterName ?? "unknown")"
        )
        ...
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    ...
                    let existingSubjects = try self.backgroundContext.fetch(
                        fetchRequest
                    )
                    let isDuplicate = !existingSubjects.isEmpty

                    self.logger.debug(
                        "📊 Duplicate check result for '\(name)': \(isDuplicate)"
                    )
                    continuation.resume(returning: isDuplicate)

                } catch {
                    let wrappedError = SubjectRepositoryError.coreDataError(
                        "Failed to check for duplicate: \(error.localizedDescription)"
                    )
                    self.logger.error("❌ \(wrappedError.localizedDescription)")
                    continuation.resume(throwing: wrappedError)
                }
