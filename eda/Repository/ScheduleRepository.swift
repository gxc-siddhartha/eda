...
class ScheduleRepository {
    ...
    private let logger = Logger(subsystem: "com.eda.app", category: "ScheduleRepository")
    ...
    func createSchedule(scheduleData: ScheduleData, for subject: Subject) async throws -> Schedule {
        self.logger.debug("📅 Creating schedule for subject: \(subject.subjectName ?? "unknown")")
            ...
            backgroundContext.perform {
                do {
                    ...
                    guard let backgroundSubject = try? self.backgroundContext.existingObject(with: subject.objectID) as? Subject else {
                        let error = ScheduleRepositoryError.subjectNotFound("Subject not found in background context")
                    ...
                    guard scheduleData.endTime > scheduleData.startTime else {
                        let error = ScheduleRepositoryError.invalidScheduleData("End time must be after start time")
                        self.logger.error("❌ \(error.localizedDescription)")
                        continuation.resume(throwing: error)
                        return
                    }
                    ...
                    let mainContextSchedule = try self.persistentContainer.viewContext.existingObject(with: newSchedule.objectID) as! Schedule
                    
                    self.logger.info("✅ Schedule created successfully for subject: \(backgroundSubject.subjectName ?? "unknown")")
                    continuation.resume(returning: mainContextSchedule)
                    
                } catch let error as ScheduleRepositoryError {
                    continuation.resume(throwing: error)
                } catch {
                    let wrappedError = ScheduleRepositoryError.coreDataError("Failed to create schedule: \(error.localizedDescription)")
    ...
    func updateSchedule(scheduleId: UUID, scheduleData: ScheduleData, for subject: Subject) async throws -> Schedule {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    ...
                    scheduleFetchRequest.predicate = NSPredicate(format: "scheduleId == %@", scheduleId as CVarArg)
                    scheduleFetchRequest.fetchLimit = 1
                    
                    let schedules = try self.backgroundContext.fetch(scheduleFetchRequest)
                    guard let schedule = schedules.first else {
                        let error = ScheduleRepositoryError.scheduleNotFound("Schedule with ID \(scheduleId.uuidString) not found")
                        self.logger.error("❌ \(error.localizedDescription)")
                        continuation.resume(throwing: error)
                        return
                    }
                    ...
                    // Get object in main context
                    let mainContextSchedule = try self.persistentContainer.viewContext.existingObject(with: schedule.objectID) as! Schedule
                    
                    self.logger.info("✅ Schedule updated successfully: \(scheduleId.uuidString)")
                    continuation.resume(returning: mainContextSchedule)
                    
                } catch let error as ScheduleRepositoryError {
                    continuation.resume(throwing: error)
                } catch {
                    let wrappedError = ScheduleRepositoryError.coreDataError("Failed to update schedule: \(error.localizedDescription)")
                    self.logger.error("❌ \(wrappedError.localizedDescription)")
                    continuation.resume(throwing: wrappedError)
                }
            }
        ...
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    ...
                    fetchRequest.predicate = NSPredicate(format: "scheduleId == %@", scheduleId as CVarArg)
                    fetchRequest.fetchLimit = 1
                    
                    let schedules = try self.backgroundContext.fetch(fetchRequest)
                    guard let schedule = schedules.first else {
                        let error = ScheduleRepositoryError.scheduleNotFound("Schedule with ID \(scheduleId.uuidString) not found")
                    ...
                    let subjectName = schedule.subject?.subjectName ?? "Unknown Subject"
                    ...
                    self.logger.info("✅ Schedule deleted successfully for subject: \(subjectName)")
                    continuation.resume()
                    
                } catch let error as ScheduleRepositoryError {
                    continuation.resume(throwing: error)
                } catch {
                    let wrappedError = ScheduleRepositoryError.coreDataError("Failed to delete schedule: \(error.localizedDescription)")
                    self.logger.error("❌ \(wrappedError.localizedDescription)")
                    continuation.resume(throwing: wrappedError)
                }
            }
    ...
    private func findActiveSchedule(
        ...
        for schedule in schedules {
            guard let startTime = schedule.scheduleStartTime,
                  let endTime = schedule.scheduleEndTime else {
                continue
            }
            ...
            if currentTotalMinutes >= startTotalMinutes && currentTotalMinutes <= endTotalMinutes {
                ...
                let mainContextSchedule = try persistentContainer.viewContext.existingObject(
    ...
    func getScheduleById(_ id: UUID) async throws -> Schedule? {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    let fetchRequest: NSFetchRequest<Schedule> = Schedule.fetchRequest()
                    fetchRequest.predicate = NSPredicate(format: "scheduleId == %@", id as CVarArg)
                    fetchRequest.fetchLimit = 1
                    
                    let backgroundSchedules = try self.backgroundContext.fetch(fetchRequest)
                    
                    if let backgroundSchedule = backgroundSchedules.first {
                        let mainContextSchedule = try self.persistentContainer.viewContext.existingObject(with: backgroundSchedule.objectID) as! Schedule
                        
                        self.logger.info("✅ Found schedule: \(mainContextSchedule.subject?.subjectName ?? "unknown subject")")
                        continuation.resume(returning: mainContextSchedule)
                    } else {
                        self.logger.info("ℹ️ No schedule found with ID: \(id.uuidString)")
                        continuation.resume(returning: nil)
                    }
                    
                } catch {
                    let wrappedError = ScheduleRepositoryError.coreDataError("Failed to fetch schedule by ID: \(error.localizedDescription)")
                    self.logger.error("❌ \(wrappedError.localizedDescription)")
                    continuation.resume(throwing: wrappedError)
                }
            }
    ...
    func getAllSchedulesForSubject(_ subjectId: UUID) async throws -> [Schedule] {
        self.logger.debug("📚 Fetching all schedules for subject: \(subjectId.uuidString)")
        
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    let fetchRequest: NSFetchRequest<Schedule> = Schedule.fetchRequest()
                    fetchRequest.predicate = NSPredicate(format: "subject.subjectId == %@", subjectId as CVarArg)
                    ...
                    self.logger.info("✅ Fetched \(mainContextSchedules.count) schedules for subject")
                    continuation.resume(returning: mainContextSchedules)
                    
                } catch {
                    let wrappedError = ScheduleRepositoryError.coreDataError("Failed to fetch schedules for subject: \(error.localizedDescription)")
                    self.logger.error("❌ \(wrappedError.localizedDescription)")
                    continuation.resume(throwing: wrappedError)
                }
            }
    ...
    func checkForTimeConflict(scheduleData: ScheduleData, in semester: Semester, excluding excludedScheduleId: UUID? = nil) async throws -> Bool {
        self.logger.debug("🔍 Checking for time conflicts on \(scheduleData.day)")
        ...
        return try await withCheckedThrowingContinuation { continuation in
            backgroundContext.perform {
                do {
                    ...
                    predicateComponents.append(NSPredicate(format: "scheduleDay == %@", scheduleData.day))
                    predicateComponents.append(NSPredicate(format: "subject.semester == %@", backgroundSemester))
                    
                    if let excludedId = excludedScheduleId {
                        predicateComponents.append(NSPredicate(format: "scheduleId != %@", excludedId as CVarArg))
                    }
                    
                    fetchRequest.predicate = NSCompoundPredicate(andPredicateWithSubpredicates: predicateComponents)
                    
                    let existingSchedules = try self.backgroundContext.fetch(fetchRequest)
                    
                    // Check for time overlaps
                    let hasConflict = existingSchedules.contains { schedule in
                        guard let existingStart = schedule.scheduleStartTime,
                              let existingEnd = schedule.scheduleEndTime else {
                            return false
                        }
                        
                        // Check if time ranges overlap
                        return !(scheduleData.endTime <= existingStart || scheduleData.startTime >= existingEnd)
                    }
                    
                    self.logger.debug("📊 Time conflict check result: \(hasConflict)")
                    continuation.resume(returning: hasConflict)
                    
                } catch {
                    let wrappedError = ScheduleRepositoryError.coreDataError("Failed to check for time conflict: \(error.localizedDescription)")
                    self.logger.error("❌ \(wrappedError.localizedDescription)")
                    continuation.resume(throwing: wrappedError)
                }
            }
