...
class SemesterViewModel: ObservableObject {
    ...
    private func setupInitialDates() {
        self.semesterEndDate =
            Calendar.current.date(
                byAdding: .month,
                value: 5,
                to: self.semesterStartDate
            ) ?? self.semesterStartDate
    ...
    func initialize() async {
        ...
        do {
            ...
            logger.error(
                "❌ SemesterViewModel initialization failed: \(error.localizedDescription)"
            )
    ...
    func loadSelectedSemester() async throws {
        ...
        do {
            ...
            if let semester = semester {
                logger.info(
                    "✅ Selected semester loaded: \(semester.semesterName ?? "unnamed")"
                )
            } else {
            ...
            let wrappedError = await wrapError(
                error,
                context: "loading selected semester"
            )
            logger.warning(
                "⚠️ Failed to load selected semester: \(wrappedError.localizedDescription)"
            )
    ...
    func createSemester() async throws -> Semester {
        ...
        do {
            ...
            let isDuplicate = try await withTimeout(operationTimeout) {
                try await self.repository.checkForDuplicateSemesterName(
                    self.selectedSemesterName
                )
            }

            if isDuplicate {
                let error = SemesterViewModelError.duplicateName(
            ...
            let semesterData = SemesterData(
                name: selectedSemesterName.trimmingCharacters(
                    in: .whitespacesAndNewlines
                ),
                startDate: semesterStartDate,
                endDate: semesterEndDate,
                passingPercentage: passingPercentage.isEmpty
                    ? nil : passingPercentage
            )

            // Create semester
            let newSemester = try await withTimeout(operationTimeout) {
                try await self.repository.createSemester(
    ...
    func updateSemester() async {
        ...
        do {
            let updated = try await repository.updateSemester(
                semesterId: id,
                semesterData: updatedData
            )
            ...
            logger.error(
                "❌ Failed to update semester: \(error.localizedDescription)"
            )
    ...
    func selectSemester(_ semester: Semester) async throws {
        logger.info(
            "🎯 Selecting semester: \(semester.semesterName ?? "unnamed")"
        )
        loadingState = .loading
    ...
    func deleteSemester(_ semester: Semester) async throws {
        guard let semesterId = semester.semesterId else {
            let error = SemesterViewModelError.semesterNotFound(
                "Invalid semester ID"
            )
        ...
        do {
            ...
            let isSelectedSemester =
                selectedSemesterForUser?.semesterId == semesterId
    ...
    func refreshData() async {
        ...
        do {
            ...
            await showSuccess(
                ...
                message: "Semester data has been updated."
            )
        } catch {
            logger.error(
    ...
    private func validateSemesterData() async throws {
        ...
        let trimmedName = selectedSemesterName.trimmingCharacters(
            in: .whitespacesAndNewlines
        )

        guard !trimmedName.isEmpty else {
            throw SemesterViewModelError.validationFailed(
        ...
        guard trimmedName.count >= 2 else {
            throw SemesterViewModelError.validationFailed(
                "Semester name must be at least 2 characters long"
            )
        }

        guard trimmedName.count <= 50 else {
            throw SemesterViewModelError.validationFailed(
                "Semester name cannot exceed 50 characters"
            )
        }
        ...
        let fiveYearsAgo =
            Calendar.current.date(byAdding: .year, value: -5, to: currentDate)
            ?? currentDate
        ...
        guard
            semesterStartDate >= fiveYearsAgo
                && semesterStartDate <= fiveYearsFromNow
        else {
            throw SemesterViewModelError.validationFailed(
                "Start date must be within 5 years of current date"
            )
        }

        guard
            semesterEndDate >= fiveYearsAgo
                && semesterEndDate <= fiveYearsFromNow
        else {
            throw SemesterViewModelError.validationFailed(
                "End date must be within 5 years of current date"
            )
        }

        if !passingPercentage.isEmpty {
            guard let percentage = Double(passingPercentage),
    ...
    var isFormValid: Bool {
        ...
        let trimmedPassingPercentage = passingPercentage.trimmingCharacters(
            in: .whitespacesAndNewlines
        )
        return !trimmedName.isEmpty && trimmedName.count >= 2
            && trimmedName.count <= 50 && Int(trimmedPassingPercentage) != nil
