...
class SubjectViewModel: ObservableObject {
    ...
    let availableIcons: [String] = [
        "book.closed", "character.book.closed", "apple.intelligence",
        "apple.logo", "applescript", "book", "graduationcap", "icloud",
        "network", "globe",
        "atom", "flask", "paintbrush", "music.note", "theatermasks", "dpad",
        "gamecontroller", "moped", "apple.terminal", "tray.full", "volleyball",
        "gearshape.2", "gyroscope", "pianokeys", "wrench.and.screwdriver",
        "printer", "suitcase", "lightbulb.max", "powercord",
    ]
    ...
    func initialize(with semester: Semester?) async {
        logger.info(
            "🚀 Initializing SubjectViewModel with semester: \(semester?.semesterName ?? "nil")"
        )
        ...
        do {
            ...
            logger.info(
                "✅ SubjectViewModel initialization completed successfully"
            )

        } catch {
            logger.error(
                "❌ SubjectViewModel initialization failed: \(error.localizedDescription)"
            )
    ...
    /// Creates a unique cache key for semester
     func subjectCacheKey(for semester: Semester?) -> String {
    ...
    private func getCachedSubjects(for key: String) -> [Subject]? {
        guard isSubjectCacheValid(for: key), let subjects = subjectCache[key]
        else {
            return nil
        }
        logger.debug(
            "📋 Using cached subjects for key: \(key) (\(subjects.count) items)"
        )
        return subjects
    }

    /// Invalidates cache for a specific key
    func invalidateSubjectCache(for key: String) {
    ...
    private func clearExpiredSubjectCache() {
        ...
        let expiredKeys = subjectCacheTimestamp.compactMap { key, timestamp in
            now.timeIntervalSince(timestamp) > subjectCacheValidityDuration
                ? key : nil
        }
        ...
        if !expiredKeys.isEmpty {
            logger.debug(
                "🧹 Cleared \(expiredKeys.count) expired subject cache entries"
            )
        }
    }
    ...
    func loadSubjects(semester: Semester?) async throws {
        ...
        if let cachedSubjects = getCachedSubjects(for: key) {
            ...
            loadingState = .success
            logger.info(
                "✅ loadSubjects: Used cached data (\(cachedSubjects.count) subjects)"
            )
            return
        }
        ...
        logger.info(
            "📚 Loading subjects for semester: \(semester!.semesterName ?? "unknown")"
        )
        loadingState = .loading
        ...
        do {
            ...
            logger.info(
                "✅ loadSubjects: Successfully loaded and cached \(subjects.count) subjects"
            )
    ...
    func createSubject(semester: Semester?) async throws -> Subject {
        ...
        do {
            ...
            let isDuplicate = try await withTimeout(operationTimeout) {
                try await self.repository.checkForDuplicateSubjectName(
                    self.subjectName,
                    in: semester!
                )
            }
            ...
            let subjectData = SubjectData(
                name: subjectName.trimmingCharacters(
                    in: .whitespacesAndNewlines
                ),
                teacher: subjectTeacher.trimmingCharacters(
                    in: .whitespacesAndNewlines
                ),
                color: subjectColor,
                icon: subjectIcon
            ...
            let newSubject = try await withTimeout(operationTimeout) {
                try await self.repository.createSubject(
    ...
    func updateSubject(semester: Semester?) async throws -> Subject {
        ...
        guard let editingSubject = editingSubject,
            let subjectId = editingSubject.subjectId
        else {
            let error = SubjectViewModelError.subjectNotFound(
                "No subject selected for editing"
            )
            await handleError(error, operation: "update subject")
            throw error
        }
        ...
        do {
            ...
            let updatedSubject = try await withTimeout(operationTimeout) {
                try await self.repository.updateSubject(
                    subjectId: subjectId,
                    subjectData: subjectData,
                    for: semester!
                )
            ...
            if let index = subjectsList.firstIndex(where: {
                ...
                logger.debug(
                    "✅ Updated subject in local list: \(updatedSubject.subjectName ?? "unnamed")"
                )
            }
            ...
            self.masterViewModel.alertMessage =
                "\(subjectName) was successfully updated."
            ...
            logger.info(
                "✅ Subject updated successfully: \(updatedSubject.subjectName ?? "unnamed")"
            )
    ...
    func deleteSubject(_ subject: Subject) async throws {
        ...
        do {
            ...
            await showSuccess(
                ...
                message:
                    "'\(subject.subjectName ?? "Subject")' has been deleted successfully."
            )

            logger.info(
                "✅ Subject deleted successfully: \(subject.subjectName ?? "unnamed")"
            )

        } catch {
            let wrappedError = await wrapError(
                error,
                context: "deleting subject"
            )
    ...
    func refreshData(semester: Semester?) async {
        ...
        do {
            ...
            await showSuccess(
                ...
                message: "Subject data has been updated."
            )
        } catch {
            logger.error(
    ...
    func startEditing(_ subject: Subject) {
        logger.info(
            "✏️ Starting to edit subject: \(subject.subjectName ?? "unnamed")"
        )
    ...
    private func validateSubjectData() async throws {
        ...
        let trimmedName = subjectName.trimmingCharacters(
            in: .whitespacesAndNewlines
        )
        let trimmedTeacher = subjectTeacher.trimmingCharacters(
            in: .whitespacesAndNewlines
        )

        guard !trimmedName.isEmpty else {
            throw SubjectViewModelError.validationFailed(
        ...
        guard trimmedTeacher.count >= 2 else {
            throw SubjectViewModelError.validationFailed(
                "Teacher name must be at least 2 characters long"
            )
        }
    ...
    var isFormValid: Bool {
        ...
        return !trimmedName.isEmpty && trimmedName.count >= 2
            && trimmedName.count <= 50 && !trimmedTeacher.isEmpty
            && trimmedTeacher.count >= 2 && trimmedTeacher.count <= 50
