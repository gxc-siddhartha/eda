...
extension AttendanceViewModel {
    ...
    func getWeeklyRunningPercentageData(for weekStartDate: Date? = nil)
        -> WeeklyAttendanceChartData
    {
        ...
        let preWeekAttendance = getAttendanceBeforeWeek(
            weekDates.start,
            from: chronologicalAttendance
        )
        ...
        if runningTotal > 0 {
            previousPercentage =
                (Double(runningPresent) / Double(runningTotal)) * 100.0
        }

        // Process each day of the week
        for (_, date) in weekDates.weekdays.enumerated() {
            let dayAttendance = getAttendanceForSpecificDate(
                date,
                from: chronologicalAttendance
            )
            var dayPresent = dayAttendance.filter {
                $0.attendancePresence == "Present"
            }.count
            let dayMedical = dayAttendance.filter {
                $0.attendancePresence == "Medical Substitution"
            }.count
            dayPresent = dayPresent + dayMedical
            let dayAbsent = dayAttendance.filter {
            ...
            logger.debug(
                "📈 Day \(point.dayName): \(point.formattedPercentage) (\(dayTotal) sessions)"
            )
        }

        let weekSummary = calculateWeekSummary(
            chartPoints: chartPoints,
            preWeekPercentage: preWeekAttendance.total > 0
                ? (Double(preWeekAttendance.present)
                    / Double(preWeekAttendance.total)) * 100.0 : nil
        )
    ...
    private func getCurrentWeekDates(startingFrom date: Date?) -> (
        start: Date, end: Date, weekdays: [Date]
    ) {
        ...
        return
            ...
            .sorted { first, second in
                guard let firstDate = first.attendanceDate,
                    let secondDate = second.attendanceDate
                else {
                    return false
                }
    ...
    private func getAttendanceBeforeWeek(
        _ weekStart: Date,
        from attendance: [Attendance]
    ) -> (total: Int, present: Int, absent: Int) {
        let beforeWeek = attendance.filter { record in
            guard let attendanceDate = record.attendanceDate else {
                return false
            }
            return attendanceDate < weekStart
        }

        let present = beforeWeek.filter { $0.attendancePresence == "Present" }
            .count
        let medical = beforeWeek.filter {
            $0.attendancePresence == "Medical Substitution"
        }.count
        let absent = beforeWeek.filter { $0.attendancePresence == "Absent" }
            .count

        return (
            total: present + absent + medical, present: present + medical,
            absent: absent
        )
    }

    private func getAttendanceForSpecificDate(
        _ date: Date,
        from attendance: [Attendance]
    ) -> [Attendance] {
        let calendar = Calendar.current
    ...
    private func calculateWeekSummary(
        chartPoints: [RunningPercentagePoint],
        preWeekPercentage: Double?
    ) -> WeekSummary {
        ...
        if totalSessions > 0 {
            weekSuccessRate =
                (Double(presentSessions) / Double(totalSessions)) * 100.0
        } else {
            weekSuccessRate = 0.0
        }
    ...
    private func determineWeeklyTrend(chartPoints: [RunningPercentagePoint])
        -> WeeklyTrend
    {
        ...
        if let first = pointsWithData.first, let last = pointsWithData.last {
            ...
            if abs(overallChange) < 1.0 {
                ...
                return negativeChanges >= positiveChanges
                    ? .declining : .volatile
            }
        }
    ...
    func getDetailedSummary(for chartData: WeeklyAttendanceChartData) -> String
    {
        ...
        description +=
            "(\(summary.presentSessionsThisWeek) present, \(summary.absentSessionsThisWeek) absent). "
        ...
        if abs(summary.totalPercentageChange) > 0.5 {
            let direction =
                summary.totalPercentageChange > 0 ? "improved" : "declined"
            description +=
                "You \(direction) by \(String(format: "%.1f", abs(summary.totalPercentageChange)))% this week."
        } else {
