...
class MasterViewModel: ObservableObject {
    ...
    private let logger = Logger(subsystem: "com.eda.app", category: "MasterViewModel")
    ...
    func importSubjectsFromSelectedFile(for semester: Semester) {
        guard let content = selectedSubjectCSVContent else {
            showErrorAlert(title: "No File Selected", message: "Please select a CSV file before importing subjects.")
            return
        }
    ...
    func importSubjectsCSV(content: String, fileName: String, for semester: Semester) {
        guard !isImporting else {
            logger.warning("⚠️ Import already in progress, ignoring new import request")
            return
        }
        
        logger.info("🚀 Starting subject CSV import from: \(fileName) for semester: \(semester.semesterName ?? "unknown")")
        ...
        Task {
            await performSubjectImport(content: content, fileName: fileName, for: semester)
        }
    }
    ...
    func importSchedulesFromSelectedFile(for semester: Semester) {
        guard let content = selectedScheduleCSVContent else {
            showErrorAlert(title: "No File Selected", message: "Please select a CSV file before importing schedules.")
            return
        }
    ...
    func importSchedulesCSV(content: String, fileName: String, for semester: Semester) {
        guard !isImporting else {
            logger.warning("⚠️ Import already in progress, ignoring new import request")
            return
        }
        
        logger.info("🚀 Starting schedule CSV import from: \(fileName) for semester: \(semester.semesterName ?? "unknown")")
        ...
        Task {
            await performScheduleImport(content: content, fileName: fileName, for: semester)
        }
    }
    ...
    func cancelImport() {
        ...
        if isImporting {
            ...
            importState = .idle
            showErrorAlert(title: "Import Cancelled", message: "The import operation was cancelled.")
        }
    }
    ...
    func searchSubjects(byName name: String, in semester: Semester? = nil) async throws -> [Subject] {
    ...
    ///   - semester: Optional semester to scope the search
    /// - Returns: Array of matching subjects
    func searchSubjects(byPartialName partialName: String, in semester: Semester? = nil) async throws -> [Subject] {
        logger.debug("🔍 Searching for subjects containing: '\(partialName)'")
        return try await bulkImporter.getSubjectsByPartialName(partialName, in: semester)
    }
    ...
    func requestFileSystemAccess() {
        ...
        guard let documentsURL = FileManager.default.urls(for: .documentDirectory,
            ...
            showErrorAlert(title: "File System Error", message: "Could not access Documents directory.")
            return
        }
        ...
        do {
            ...
            logger.info("✅ Successfully accessed Documents directory. Found \(files.count) items")
            ...
            for fileURL in files {
                do {
                    ...
                    logger.debug("\(type): \(fileURL.lastPathComponent) (\(fileSize) bytes, modified: \(modDate))")
                } catch {
                    logger.debug("📄 File: \(fileURL.lastPathComponent) (could not read properties)")
                }
            }
            
            // Show success alert with file count
            let message = files.isEmpty ?
                "Documents directory is empty." :
                "Found \(files.count) items in Documents directory. Check console for details."
            
            showSuccessAlert(title: "File System Access Successful", message: message)
            
        } catch {
            logger.error("❌ File system access denied: \(error.localizedDescription)")
            showErrorAlert(title: "File System Access Denied", message: "Could not access Documents directory: \(error.localizedDescription)")
        }
    }
    ...
    func listCSVFilesInDocuments() -> [URL] {
        ...
        do {
            let allFiles = try FileManager.default.contentsOfDirectory(at: documentsURL,
            ...
            logger.info("📊 Found \(csvFiles.count) CSV files in Documents directory")
            return csvFiles
            
        } catch {
            logger.error("❌ Error listing CSV files: \(error.localizedDescription)")
            return []
        }
    }
    ...
    private func performSubjectImport(content: String, fileName: String, for semester: Semester) async {
        importProgress = "Reading subject CSV content..."
        
        do {
            importProgress = "Processing subjects..."
            let result = try await bulkImporter.bulkImportSubjects(content: content, fileName: fileName, for: semester)
            ...
            showErrorAlert(title: "Subject Import Failed", message: errorMessage)
            
            logger.error("❌ Subject import failed: \(error.localizedDescription)")
        }
    ...
    private func performScheduleImport(content: String, fileName: String, for semester: Semester) async {
        importProgress = "Reading schedule CSV content..."
        
        do {
            importProgress = "Processing schedules..."
            let result = try await bulkImporter.bulkImportSchedules(content: content, fileName: fileName, for: semester)
            ...
            importState = .failed(error)
            let errorMessage = createErrorMessage(from: error)
            showErrorAlert(title: "Schedule Import Failed", message: errorMessage)
            
            logger.error("❌ Schedule import failed: \(error.localizedDescription)")
        }
    ...
    private func createSuccessMessage(from result: BulkImportResult, type: String) -> String {
        var message = "Successfully imported \(result.successfulImports) out of \(result.totalRows) \(type)."
    ...
    private func createErrorMessage(from error: Error) -> String {
        if let bulkImportError = error as? BulkImportRepositoryError {
            ...
            case .csvParsingError(let details):
                return "Failed to read the CSV file. Please check that it's a valid CSV format.\n\nDetails: \(details)"
            case .semesterNotFound(let details):
                return "The selected semester could not be found. Please try again.\n\nDetails: \(details)"
            case .coreDataError(let details):
                return "A database error occurred while importing. Please try again.\n\nDetails: \(details)"
            case .invalidRowData(let details, let rowNumber):
                return "Invalid data found in row \(rowNumber). Please check your CSV file.\n\nDetails: \(details)"
            }
        }
