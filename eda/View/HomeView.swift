...
struct HomeView: View {
    ...
    var body: some View {
        if subjectViewModel.subjectsList.isEmpty {
            ...
            VStack {
                ...
                .confirmationDialog(
                    "Add Components",
                    isPresented: $presentConfirmationDialog
                ) {
                    Button("Add Subject") {
                        subjectViewModel.presentSubjectCreateSheet = true
                    }
                    ...
                    if !subjectViewModel.subjectsList.isEmpty {
                        ...
                        Button("Add Attendance Log") {
                            attendanceViewModel.presentAttendanceCreateSheet =
                                true
                        }
                    }
            ...
            List {
                ...
                if !subjectViewModel.subjectsList.isEmpty {
                    Section("My Subjects") {
                        ForEach(
                            $subjectViewModel.subjectsList.wrappedValue,
                            id: \.subjectId
                        ) { subject in
                            NavigationLink(
                                destination: AttendanceView(subject: subject)
                            ) {
                            ...
                            .swipeActions(edge: .leading) {
                                Button {
                                    ...
                                    subjectViewModel.presentSubjectEditSheet =
                                        true
                                } label: {
                                    Text("Edit")
                                }
            ...
            .alert(isPresented: $showDeleteSubjectAlert) {
                Alert(
                    ...
                    primaryButton: .destructive(Text("Delete")) {
                        Task {
                            do {
                                try await subjectViewModel.deleteSubject(
                                    selectedSubjectForDeletion!
                                )

                                // Refresh schedules since subject deletion affects them
                                if let semester = semesterViewModel
                                    .selectedSemesterForUser
                                {
                                    ...
                                    await scheduleViewModel
                                        .checkCurrentlyActiveSchedule(
                                            for: semester
                                        )
                                }
                            } catch {
            ...
            .task {
                if !subjectViewModel.isInitialized {
                    if let semester = semesterViewModel.selectedSemesterForUser
                    {
                        ...
                        semesterViewModel.selectedSemesterForUser =
                            firstSemester
    ...
    private var toolbarContent: some ToolbarContent {
        ...
        ToolbarItem(placement: .topBarLeading) {
            if semesterViewModel.semesterList.isEmpty
                || semesterViewModel.selectedSemesterForUser == nil
            {
                addSemesterButton
            } else {
    ...
    private var semesterSelectionMenu: some View {
        Menu {
            ...
            HStack {
                Text(
                    semesterViewModel.selectedSemesterForUser?.semesterName
                        ?? "Select Semester"
                )
                .lineLimit(1)
    ...
    private func refreshAllData(for semester: Semester) async {
        logger.debug(
            "🔄 Refreshing all data for semester: \(semester.semesterName ?? "unknown")"
        )
        ...
        await withTaskGroup(of: Void.self) { group in
            ...
            group.addTask {
                await scheduleViewModel.checkCurrentlyActiveSchedule(
                    for: semester
                )
            }
        }
    ...
    private func refreshActiveSchedule() async {
        guard let semester = semesterViewModel.selectedSemesterForUser else {
            return
        }
