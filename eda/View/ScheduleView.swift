...
struct ScheduleView: View {
    ...
    var body: some View {
        ...
        List {
            ...
            Section {
                ForEach(
                    scheduleViewModel.schedulesForSelectedDate,
                    id: \.scheduleId
                ) { schedule in

                    ScheduleViewListItem(schedule: schedule).listRowSeparator(
                        .hidden
                    )
                    .padding(.trailing)
            ...
            .navigationTitle(Text("Schedules")).navigationBarTitleDisplayMode(
                .large
            )
            .onAppear {
                if semesterViewModel.selectedSemesterForUser != nil {
            ...
            .alert(isPresented: $showDeleteScheduleAlert) {
                Alert(
                    ...
                    message: Text(
                        "This will also remove all the attendances for this schedule. Are you sure you want to delete this schedule?"
                    ),
                    primaryButton: .destructive(Text("Delete")) {
                        Task {
                            do {
                                try await scheduleViewModel.deleteSchedule(
                                    scheduleToDelete!
                                )
